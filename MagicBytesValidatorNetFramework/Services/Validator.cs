...
namespace MagicBytesValidatorNetFramework.Services
{
    public class Validator : IValidator
    {
        ...
        /// <returns>Returns  if the Stream matches one of the FileType's magic-byte sequences.</returns>
        ...
        )
        {
            ...
            await fileStream.ReadAsync(
                streamBytes, 0, maxLengthFileTypeMagicByteSequences,
                cancellationToken
                );
        ...
        /// <param name="fileType">FileType that the stream should be validated against</param>
        /// <returns>Returns  if the Stream matches one of the FileType's magic-byte sequences.</returns>
        public bool IsValid(Stream fileStream, FileType fileType) 
        {
            var maxLengthFileTypeMagicByteSequences = fileType.MagicByteSequences.Max(mb => mb.Length);
            var streamBytes = new byte[maxLengthFileTypeMagicByteSequences];
            ...
            fileStream.Read(streamBytes, 0, maxLengthFileTypeMagicByteSequences);
